...
async function main() {
  ...
  for (const name of ['CaseStatement']) {
    ...
    console.log(`=== ${name} ===`);
    // Find all zero spans in port
    const portStr = JSON.stringify(json);
    const refStr = JSON.stringify(ref);
    
    // Show each decl's annotation span
    for (let i = 0; i < json.decls.length; i++) {
      const pd = json.decls[i];
      const rd = ref.decls[i];
      const pspan = pd.annotation && pd.annotation.sourceSpan;
      const rspan = rd && rd.annotation && rd.annotation.sourceSpan;
      const pid = pd.identifier || '(rec)';
      const rid = rd && (rd.identifier || '(rec)');
      const pMatch = pspan && (pspan.start[0] === 0 || pspan.end[0] === 0) ? ' ZERO' : '';
      console.log(`  decl[${i}] ref:${rid}(${rspan&&JSON.stringify(rspan.start)}-${rspan&&JSON.stringify(rspan.end)}) port:${pid}(${pspan&&JSON.stringify(pspan.start)}-${pspan&&JSON.stringify(pspan.end)})${pMatch}`);
    }
  }
}
